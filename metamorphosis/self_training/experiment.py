...
class Experiment:
    ...
    def run_iteration(self) -> bool:
        ...
        """
        self._train_multiclass_dt()
        dt_acc = self._test_multiclass_dt()
    ...
    # added multiclass DT
    def _train_multiclass_dt(self) -> None:
        """
        Train a scikit-learn DecisionTreeClassifier using the same train data as the CNN.
        """
        # Initialize arrays to store the training data and labels
        X_train = []
        y_train = []

        # Iterate over the train_loader to get the data in NumPy format
        train_loader = DataLoader(self.train_subset, batch_size=64, shuffle=True)
        for data, target in train_loader:
            # Flatten the image tensors (from 1x28x28 to 784 for each image)
            data = data.view(data.size(0), -1).numpy()  # Convert to NumPy
            target = target.numpy()  # Convert labels to NumPy
            X_train.append(data)
            y_train.append(target)

        # Concatenate all batches to form the full dataset
        X_train = np.concatenate(X_train, axis=0)  # Shape: (num_samples, 784)
        y_train = np.concatenate(y_train, axis=0)  # Shape: (num_samples,)

        # Create and train the Decision Tree Classifier
        self.dt_clf = DecisionTreeClassifier()  # You can customize hyperparameters
        self.dt_clf.fit(X_train, y_train)  # Train the decision tree
        print("Decision Tree trained successfully!")

    def _test_multiclass_dt(self) -> float:
        """
        Test the scikit-learn DecisionTreeClassifier on the test set and return accuracy.
        """
        # Initialize arrays to store the test data and labels
        X_test = []
        y_test = []
